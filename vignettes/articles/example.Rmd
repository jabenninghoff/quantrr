...
Import and validate data. The data was collected from 3 Technology SMEs, 3 Business SMEs, and one
...
```{r import}
# note: widget_risks <- read_xlsx("widget.xlsx", sheet = "Risks")
risks <- widget_risks |>
  clean_names()
...
# note: widget_estimates <- read_xlsx("widget.xlsx", sheet = "Estimates")
estimates <- widget_estimates |>
  clean_names() |>
  rename(
    lambda = frequency_per_yer,
    p05 = low_5_percent, p95 = high_95_percent, p50 = most_likely
  )
